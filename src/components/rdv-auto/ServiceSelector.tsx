...
import { Wrench, Hammer, ClipboardList } from 'lucide-react'
import { SERVICES, Service } from '@/lib/rdv-store'
  ...
  return (
    <div className="space-y-3">
      {SERVICES.map((service, i) => {
        const { Icon, glow, selected: selectedCls, iconBg, iconColor, badge } = META[service.id]
          ...
          >
            ...
            {isSelected && (
              <motion.div
                ...
                style={{
                  background: `radial-gradient(ellipse at 20% 50%, ${glow} 0%, transparent 65%)`,
                }}
              />
            )}
            ...
            >
              <Icon
                className={`w-5 h-5 transition-colors duration-300 ${isSelected ? iconColor : 'text-white/30'}`}
              />
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p
                className={`text-sm leading-snug transition-colors duration-300 ${
                  isSelected ? 'text-white/55' : 'text-white/25'
                }`}
              >
                {service.description}
              </p>
            ...
            <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
              <span
                className={`
                  text-[11px] px-2.5 py-1 rounded-full font-mono border transition-all duration-300
                  ${isSelected ? badge : 'bg-white/[0.04] text-white/25 border-white/[0.07]'}
                `}
              >
              ...
              <span
                className={`
                  text-[11px] px-2.5 py-1 rounded-full font-semibold border transition-all duration-300
                  ${isSelected ? badge : 'bg-white/[0.04] text-white/25 border-white/[0.07]'}
                `}
              >
